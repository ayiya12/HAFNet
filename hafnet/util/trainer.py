...
class SimplePanTrainer(metaclass=ABCMeta):
    ...
    def __init__(self, cfg):
        self.cfg = cfg
        self.logger = logging.getLogger(f"hafnet.{cfg['exp_name']}")
    ...
    def _compute_val_metrics(self, sr, gt):  
        """Override this to compute custom validation metrics"""  
        return {}  
    
    def _on_val_end(self, epoch, val_loss, val_metrics=None):  
        """Called after validation. Return True to stop training."""  
        return False
            ...
            if epoch % self.cfg['val_interval'] == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch in val_loader:
                        ...
                        if hasattr(self, '_compute_val_metrics'):  
                            batch_metrics = self._compute_val_metrics(sr, batch['gt'].to(self.dev))  
                    ...
                    if hasattr(self, '_on_val_end'):  
                        should_stop = self._on_val_end(epoch, avg_val_loss, avg_metrics)  
                        if should_stop:  
                            self.logger.info("Early stopping - breaking training loop")  
                            break
