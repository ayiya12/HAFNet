...
from hafnet.util.seed import seed_everything
from hafnet.dataset.h5pan import H5PanDataset
from hafnet.util.log import save_mat_data

@torch.no_grad()
def run_test(model_name, weight_file, test_dataset="reduced", cfg=None, preset=None, override=None, output_dir=None):
    print("[DEBUG 1] Starting run_test function.")
    module = importlib.import_module(f"hafnet.models.{model_name}")
    print("[DEBUG 2] Model module imported.")
    ...
    print("[DEBUG 3] Dataset loaded successfully.")

    trainer = module.Trainer(cfg)
    print("[DEBUG 4] Trainer initialized.")
    trainer.model.load_state_dict(torch.load(weight_file))
    trainer.model.eval()
    print("[DEBUG 5] Model weights loaded and set to eval mode.")

    sr = trainer.run_test(dataset)
    print("[DEBUG 6] Test inference finished.")
